...
class MetaConversionService
{
    ...
    public function sendAddToCart(Request $request, string $eventId): void
    {
        ...
        $value = (float) ($eventData['value'] ?? 792000);
        $currency = $eventData['currency'] ?? 'IDR';

        $content = (new Content)
            ->setProductId($eventData['product_id'] ?? 'gacf-2026')
            ->setQuantity(1);
    ...
    public function sendPurchase(Request $request, string $eventId, float $value = 0, string $currency = 'IDR'): void
    {
        ...
        $content = (new Content)
            ->setProductId('gacf-2026')
            ->setQuantity(1);

        $customData = (new CustomData)
            ->setContentName('GACF 2026')
            ->setContentType('product')
            ->setValue($value > 0 ? $value : 792000)
            ->setCurrency($currency)
            ->setContents([$content]);
